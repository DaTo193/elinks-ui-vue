...
## 多租户改造更新说明 20260412-dlh
进行好多租户改造，但仍存在以下问题：
1、不支持此应用场景（跨租户菜单隐藏了不开放）：创建一个平台级用户，给该用户添加多个租户的权限，该用户登录后，有一个整体的数据看板首页，另外有下拉单可选择进入不同租户的工作台。
针对此问题：目前考虑后续先针对平台管理员admin登录后的首页，定制开发一个综合查看所有租户的统计分析类看板和各类数据的综合性查询的菜单，比如另开发一套产品、设备菜单，可以查看所有租户的产品、设备。

2、租户登录后的”物联网-首页“中有部分链接无法进入页面，并且页面内容需要定制调整，有些内容不需了需要删除。

3、设备接入网关、协议管理、网络组件、证书管理、插件管理等均是共用的，没有按租户独立。
改造后登录分成了平台管理员登录和租户用户登录两个页面，登录地址分别如下：
平台管理员登录：http://localhost:9100/#/platform/login
租户登录：http://localhost:9100/#/login
